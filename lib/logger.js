...
exports.log = function (type, tag, message) {
    var date = new Date();
    date = util.format("%02i:%02i:%02i", date.getHours(), date.getMinutes(), date.getSeconds());
    console.log(util.format("%s %s[%s] %s%s", date, type.color, tag.toUpperCase(), message, color.RESET));
};
...
exports.system = function (type, message) {
    var date = new Date();
    date = util.format("%02i:%02i:%02i", date.getHours(), date.getMinutes(), date.getSeconds());
    console.log(util.format("%s %s[SYS] %s%s", date, type.color, message, color.RESET));
};
...
exports.response = function (to, message) {
    var date = new Date();
    date = util.format("%02i:%02i:%02i", date.getHours(), date.getMinutes(), date.getSeconds());
    console.log(util.format("%s %s[%s] %s%s", date, color.GREEN, to, message, color.RESET));
};
...
exports.debug = function (message) {
    ...
    var date = new Date();
    date = util.format("%02i:%02i:%02i", date.getHours(), date.getMinutes(), date.getSeconds());
    console.log(util.format("%s %s[DEBUG] %s%s", date, color.MAGENTA, message, color.RESET));
};